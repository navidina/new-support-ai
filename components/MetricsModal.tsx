...
const MetricsModal: React.FC<MetricsModalProps> = ({ isOpen, onClose, chunks }) => {
  ...
  const stats = useMemo(() => {
    ...
    const totalFiles = new Set(chunks.map(c => c.source.id)).size;
    ...
    const totalSize = chunks.reduce((acc, c) => acc + c.content.length, 0);
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      // Calculate Y for tooltip position
  ...
  const renderChart = () => {
      ...
      const chartHeight = height - padding * 2;
      const maxCount = stats.data[stats.data.length - 1].count || 1; // Avoid divide by zero
      ...
      return (
          <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full overflow-visible">
              ...
              {/* Data Points (visible only on ends or sparse) */}
      ...
      <div className="bg-white rounded-2xl w-full max-w-4xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
        ...
        <div className="flex items-center justify-between p-5 border-b border-slate-100 bg-slate-50">
          <div className="flex items-center gap-3">
            <div className="bg-violet-600 p-2 rounded-lg text-white shadow-lg shadow-violet-500/30">
              <BarChart2 className="w-6 h-6" />
            </div>
            <div>
              <h2 className="text-xl font-bold text-slate-800">گزارش عملکرد پایگاه دانش</h2>
              <p className="text-xs text-slate-500 mt-1">آمار و تحلیل داده‌های ایندکس شده</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-slate-50/50 custom-scrollbar">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col items-center text-center">
                    <Database className="w-8 h-8 text-blue-500 mb-2 bg-blue-50 p-1.5 rounded-lg" />
                    <span className="text-2xl font-bold text-slate-800">{stats.totalChunks}</span>
                    <span className="text-xs text-slate-500">تعداد کل قطعات (Chunks)</span>
                </div>
                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col items-center text-center">
                    <Activity className="w-8 h-8 text-emerald-500 mb-2 bg-emerald-50 p-1.5 rounded-lg" />
                    <span className="text-2xl font-bold text-slate-800">{stats.totalFiles}</span>
                    <span className="text-xs text-slate-500">اسناد پردازش شده</span>
                </div>
                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col items-center text-center">
                    <PieChart className="w-8 h-8 text-amber-500 mb-2 bg-amber-50 p-1.5 rounded-lg" />
                    <span className="text-2xl font-bold text-slate-800" dir="ltr">{stats.avgChunkSize}</span>
                    <span className="text-xs text-slate-500">میانگین کاراکتر هر قطعه</span>
                </div>
                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col items-center text-center">
                    <Calendar className="w-8 h-8 text-violet-500 mb-2 bg-violet-50 p-1.5 rounded-lg" />
                    <span className="text-sm font-bold text-slate-800 mt-2 mb-1">{stats.lastIndexed}</span>
                    <span className="text-xs text-slate-500">آخرین بروزرسانی</span>
                </div>
            </div>
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                    <div className="flex items-center justify-between mb-6">
                        <h3 className="font-bold text-slate-800 flex items-center gap-2">
                            ...
                            روند رشد پایگاه دانش
                        </h3>
                        <span className="text-xs text-slate-400 bg-slate-100 px-2 py-1 rounded">تجمعی</span>
                    </div>
                    
                    <div 
                        className="h-[300px] w-full relative group cursor-crosshair" 
                        dir="ltr"
                        ref={chartRef}
                        ...
                        {stats.data.length > 0 ? (
                            <>
                                ...
                                {tooltipData && (
                                    <>
                                        <div 
                                            className="absolute top-0 bottom-[20px] w-0.5 bg-slate-300 pointer-events-none transition-all duration-75"
                                            style={{ left: tooltipData.x }}
                                        ></div>
                                        <div 
                                            className="absolute bg-slate-800 text-white text-xs rounded-lg p-2 shadow-xl pointer-events-none transform -translate-x-1/2 -translate-y-full transition-all duration-75 z-10"
                                            style={{ left: tooltipData.x, top: tooltipData.y - 10 }}
                                        >
                                            <div className="font-bold mb-1">{tooltipData.label}</div>
                                            <div className="flex items-center gap-2">
                                                <span className="w-2 h-2 rounded-full bg-blue-400"></span>
                                                <span>{tooltipData.value} Chunks</span>
                                            </div>
                                        </div>
                                        <div 
                                            className="absolute w-3 h-3 bg-blue-600 border-2 border-white rounded-full pointer-events-none transition-all duration-75 shadow-sm"
                                            style={{ left: tooltipData.x - 6, top: tooltipData.y - 6 }}
                                        ></div>
                                    </>
                                )}
                            </>
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-100 flex justify-end">
          <button 
            onClick={onClose}
            className="px-6 py-2 bg-slate-800 hover:bg-slate-700 text-white text-sm font-medium rounded-lg transition-colors shadow-lg"
          >
            بستن گزارش
          </button>
        </div>
